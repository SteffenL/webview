...
if command -v clang-tidy >/dev/null 2>&1 ; then
	...
	find "$DIR" -type f -regextype posix-extended -iregex '.+\.cc$' -a -exec clang-tidy {} -- $CXXFLAGS \;
else
...
find "$TEST_BIN_DIR" -type f -regex "[^.]_w+" -not -name "webview_library_type_test_*" | while read f; do
	echo "Running test app: $(basename "$f")"
	"$f"
done

printf "Checking expected compiler options for shared library: "
OUTPUT=$(LD_LIBRARY_PATH="$SHARED_BUILD_LIB_DIR" DYLD_LIBRARY_PATH="$SHARED_BUILD_LIB_DIR" "$TEST_BIN_DIR/webview_library_type_test_shared")
...
printf "Checking expected compiler options for static library: "
...
printf "Checking expected compiler options for header-only library: "
