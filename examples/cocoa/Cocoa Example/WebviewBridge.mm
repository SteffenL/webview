...
- (void)setHTML:(NSString *)html {
  _webview->set_html(std::string{
      html.UTF8String, [html lengthOfBytesUsingEncoding:NSUTF8StringEncoding]});
}

- (void)navigate:(NSString *)url {
  _webview->navigate(std::string{
      url.UTF8String, [url lengthOfBytesUsingEncoding:NSUTF8StringEncoding]});
}

- (void)bindWithName:(NSString *)name
               block:(NSString * (^)(NSString *req))block {
  const std::string name_{
      name.UTF8String, [name lengthOfBytesUsingEncoding:NSUTF8StringEncoding]};
  ...
  _webview->bind(name_, [=](const std::string &req_) -> std::string {
    ...
    std::string res_{res.UTF8String,
                     [res lengthOfBytesUsingEncoding:NSUTF8StringEncoding]};
    return res_;
  });
}
